...
1. On the taskbar, Search and select **Command Prompt**.
...
1. Run the following command to run the function app project:
   ...
   > **Note**: If you're facing the error as shown in the screenshot below:
    
    ![error](./media/error.jpg)

   > Run the below command in the terminal and Open the new terminal and Perform step 2 & 3:
...
1. Update the **Run** method signature code block to change the schedule to run once every **1 minute**:
...
1. Observe the function run that occurs about every 1 minute. Each function run should render a simple message to the log.
...
1. When you receive the open command prompt, run the following command to login to the Azure Command-Line Interface (CLI):
    ...
    **Note :** If the az login command is failed to run, Install the az-cli from the given doc link `https://learn.microsoft.com/en-us/cli/azure/install-azure-cli-windows?tabs=azure-cli` open this link in a new browser and click on **Latest MSI of the Azure CLI(64-bit)** to install the az-cli. once downloaded open the file to install. then open the **Command Prompt** again and run the above two steps 2 and 3.
