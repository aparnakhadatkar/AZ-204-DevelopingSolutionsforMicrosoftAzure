...
1. On the **Access keys** blade, select **Show (2)** to view the keys. Review any one of the keys, and then copy the value of either of the **Connection string** boxes to the clipboard.
...
1. On the **Create Function App** blade, on the **Basics** tab, perform the following actions, and then select **Next: Storage (8)**:
    ...
    | **Runtime stack** drop-down list | Select **.NET (4)** |
    | **Version** drop-down list | Select **8 (LTS),  isolated worker model (5)** |
    | **Region** drop-down list | Select the **West US (6)** region |
    | **Operating System** option | Select **Linux (7)** |
...
1. On the **Storage** tab, perform the following actions, and then select **Review + create (2)**:
    ...
    | Setting                            | Action                                             |
    | ---------------------------------- | -------------------------------------------------- |
    | **Storage account** drop-down list | Select the **funcstor<inject key="DeploymentID" enableCopy="false"/> (1)** storage account|
...
1. Close all currently running instances of the **Command Prompt** application.
   ...
   >**Note:** Ensure you check this validation upon completing the lab; otherwise, the validation will fail.
...
1. In the **Upload blob** window, perform the following actions, and then select **Upload**:
    ...
    | Setting                          | Action                           |
    | -------------------------------- | -------------------------------- |
    | **Upload blob** section  | Click on **Browse for files**.    |
...
1. Close all currently running instances of the **Command Prompt** application.
 ...
 > **Congratulations** on completing the task! Now, it's time to validate it. Here are the steps:
   ...
   <validation step="048913b3-c2bb-4f2e-b373-4b7f5dd10f11" /> 
