...
1.  Close the **Startup.cs** file without making any modifications.
   ...
   > **Note**: At the path of **C:\AllFiles\AZ-204-DevelopingSolutionsforMicrosoftAzure-master\Allfiles\Labs\04\Solution\AdventureWorks\AdventureWorks.Web\Configuration\Settings.cs**. there might be some code snippet missing please check and verify the code Snippet Matches with the below code if it does not match please copy the below code and paste it in **Settings.cs** file and **Save** it.
