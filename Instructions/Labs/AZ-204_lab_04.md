...
1. Select **+ Create a resource** and search for **Azure Cosmos DB** and then in the list of results, select **Azure Cosmos DB (1)**.

1. On the marketplace page, under the Azure Cosmos DB section select **Create (2)** from the dropdown and click on **Azure Cosmos DB (3)**.

    ![](./media/3012025(14).png)

1. On the **Which API best suits your workload?** blade, Click **Create** on **Azure Cosmos DB for NoSQL**.

    ![](./media/15125(6).png)

1. On the **Basics** tab of the **Create Azure Cosmos DB Account - Azure Cosmos DB for NoSQL** page, perform the following actions, and then select **Review + create (5)**:

    | Setting   | Action |
    | ------ | ------ |
    | **Subscription**       | Retain defaults **(1)**                                            |
    | **Resource group** drop-down list  | Select existing rg: **PolyglotData-<inject key="DeploymentID" enableCopy="false"/> (2)** |
    | **AccountName** text box    | Enter **polycosmos<inject key="DeploymentID" enableCopy="false"/> (3)** |
    | **Location** drop-down list | Leave the default and make a note of it|
    | **Capacity mode** section   | Select **Serverless (4)** |
...
1. On the **Azure Cosmos DB account** blade, find the **Settings** section, and then select the **Keys (1)** link.

1. In the **Keys** pane, record the values of the **URI (2)**, **PRIMARY KEY (3)**, and **PRIMARY CONNECTION STRING (4)** text boxes into Notepad. You'll use these values later in this lab.

    ![](./media/3012025(17).png)
...
1. On the **Basics** tab of the **Create a storage account** blade, perform the following actions, and then select **Next : Advanced > (7)**:

    | Setting         | Action      |
    | -----  | -----|
    | **Subscription**             | Retain defaults **(1)** |
    | **Resource group** drop-down list        | Select **PolyglotData-<inject key="DeploymentID" enableCopy="false"/> (2)** |
    | **Storage account name** text box | Enter **polystor<inject key="DeploymentID" enableCopy="false"/> (3)**  |
    | **Region** drop-down list         | Select the same region where you created the Cosmos DB account earlier in this exercise **(4)** |
    | **Primary service**               | No changes |
    | **Performance** section           | Select **Standard (5)** |
    | **Redundancy** drop-down list     | Select **Locally-redundant storage (LRS) (6)** |

    ![](media/3012025(16).png)
...
1. Select **Create** to create the storage account by using your specified configuration.
   ...
   <validation step="11ac3d68-8d8a-40b6-b677-5e9a1bd70912" />
...
1. Click on **Shared access tokens** next and generate an access token with the default settings by clicking **Generate SAS token and URL**. Record the value of the **Blob SAS token** in Notepad, you will need this later.

    > **Note**: You should now have 5 values in Notepad. The **URI**, **Primary Key** and **Primary Connection String** for your Cosmos DB account. Then the **URL** and **SAS Token** for your Blob container.
...
1. In the **Upload blob** pop-up, perform the following actions:
   ...
   <validation step="0fd20da1-9f49-4a25-b33c-0ce8fc9cca97" />

### Task 2: Review JSON data

1. From the lab computer, start **Visual Studio Code**.

1. From the **File** menu, select **Open File**, browse to **C:\AllFiles\AZ-204-DevelopingSolutionsforMicrosoftAzuremaster\Allfiles\Labs\04\Solution\AdventureWorks\AdventureWorks.Upload**, select **models.json**, and then select **Open**.
...
1. Switch back to the **Visual Studio Code** window.
   ...
   <validation step="bc7f233a-963d-4caf-a9e1-594b2148333e" />
...
1. Switch to the **Visual Studio Code** window, and then select **Kill Terminal** (the **Recycle Bin** icon) to close the currently open terminal and any associated processes.
   ...
   <validation step="9db78bbe-f847-4db0-ad2e-945ece88429f" />
