...
In this lab, you will deploy and configure an Azure Web App that integrates with GitHub. The web app will be a simple implementation of a single-page web app accessible only to internal employees. The website will host content related to company strategies and division sales goals. To create the web app, you will use the Azure portal and connect it to a GitHub repository. To do this, you will need a free GitHub account, where you will upload the sample website content provided as part of this lab. Once the web app is up and running, you will make changes to the content and watch as the GitHub content is synced to the web app, using the CI/CD methodology. Because this is an employee-only web app, you will need to secure access to the web app by creating users in Azure AD and implementing security for the web app through user authentication.
...
- **GitHub Account**: An existing GitHub account is required.

- **Web Application**: A sample or existing web application to deploy and test features during the lab.
...
1. Navigate to your mail, you will be getting a verification code. Paste the **verfication code** **(1)** and then click on **Verify (2)**.
...
1. In the repository page on GitHub, select the **Add file (1)**, and from the drop-down list select **Upload Files (2)**.
...
1. Open the **File Explorer** window, navigate to `C:\AllFiles\AZ-204-DevelopingSolutionsforMicrosoftAzure-master\Allfiles\Labs\13\Starter\webapp-sam-lab13`, press **Enter**. select all files and folders. Then use the **drag and drop** feature, to add them to the GitHub repository.

    ![](./media/w23.png)

1. Add a commit message that describes the changes you made (**Initial commit of sample web app code**) **(1)** and select **Commit changes (2)** to upload the sample web app code to the repository.

    ![](./media/w9.png)

### Task 2: Create the web app in the Azure portal and connect it to the GitHub account

1. Navigate to the  Azure portal. Select **Create a resource**.
...
1. In the **Create Web App** blade, select the **Basics** tab and perform the following actions.
    ...
    | **Resource group** | Select **ConfidentialStack (2)** |
...
1. Select the **Monitor + secure (1)** tab and select **Yes (2)** in the **Enable Application Insights** section. Select **applicationinsight (3)** for **Application Insights**. Then select **Review + create (3)**.
...
1. Open the **Azure portal** and navigate to the **Web app** that you created in the previous task.

1. In the **Settings (1)** section, select **Configuration (2)** from the left-hand menu. On the **Configuration** blade, select **General settings**. In the **General settings** tab, select **On (3)** for **FTP Basic Auth Publishing Credentials**, select **Save (4)**.

    ![](./media/w17.png)

1. Then select **Continue** on **Save changes**.    

    ![](./media/w18.png)

    >**Note**: **Wait for a minute to save the configurations.**

1. In the **Deployment (1)** section, select **Deployment Center (2)** from the left-hand menu. In the **Settings (3)** tab, select **Source** from the drop-down list and select **GitHub (4)**.
...
1. Click on **Authorize AzureAppService**.    
...
1. Open the **GitHub repository** **`webapp`** that you created in the previous task, and select the **Actions (1)** from the top menu. On the **Actions** tab, under the **All workflows**, select the **Add or update the Azure App Service build and deployment workflow config (2)** to see the build and deployment process.

    ![](./media/w24.png)

    ![](./media/w25.png) 

1. Once the **build and deployment process** is complete, access the **Azure portal** and go to the **App Service**.
...
1. Open the **GitHub repository** **`webapp`** that contains the code for the **web app**.
...
1. Open the **Azure portal** navigate to the **App Service**, and select the **web app** you created in the previous exercise.
...
1. On the **Create a load testing resource** blade, perform the following actions, and then select **Review + Create (5)**:
    ...
    | **Resource group** drop-down list | Select **ConfidentialStack (2)** in the list |
...
### Task 5: Quick test and analyze the performance metrics of the web app

1. On the new created **Azure Load Testing** blade, in the **Tests** section, select the **Tests (1)**. Select **+ Create (2)**, and from the drop-down list, select **Create a URL-based test (3)**.
...
1. To download the load testing results, select the **Download**, from the drop-down list, select the **Results**.
...
1. Open the **Azure portal** and search for **Microsoft Entra ID (1)** and slect **Microsoft Entra ID (2)** from the services.
...
1. Open the **Azure portal** and navigate to the **App Service**, and select the **web app** you created in the previous exercise.

1. On the **Web App** blade, select **Application Insights (1)** from the **Monitoring** section and slect **applicationinsights (2)** or you can also select **View Application Insights data**.
...
1. On the **Application Insights** blade, select **Logs** under the **Monitoring** section. Close the pop-up by clicking on **X**.
...
1. Select **Run** to execute the query. The results will be displayed in a table below the query editor.
    ...
    ![](./media/w-61.png)
     ...
     >**Note**: Sometimes it might take some time to fetch the results.
...
### Task 3: View the Application Map in the Azure Web App
...
1. On the **Application Map** blade, select the circle from the center of the Application Map page.
