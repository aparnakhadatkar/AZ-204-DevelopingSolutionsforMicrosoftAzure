...
1. On the **Container registry** blade, in the **Settings** section, select **Access keys (1)**. On the **Access keys** blade, enable the **Admin user (2)**. Copy the value of **username (3)** box to the clipboard.
...
1. Open the **File Explorer** window, browse to `C:\AllFiles\AZ-204-DevelopingSolutionsforMicrosoftAzure-master\Allfiles\Labs\14\Starter\webapp-lab14\`, and then open a text editor.
...
1. Naviage to **View (1)** tab, check the box for **File name extension (2)**. Remove `.txt` from **Dockerfile** **(3)**.
...
1. On the **Basics** tab of the **Create Container Instance** page, fill out the following information (leave others with their default values), and then select **Next: Networking (10)**:
    ...
    | **Availability zones** drop-down list | Select **1 (5)** |
...
1. On the **Networking** tab, fill out the following information and select **Review + create (4)**.

    | Setting | Action |
    | -- | -- |
    | **DNS name label** text box | Enter a **dns-<inject key="DeploymentID" enableCopy="false"/> (1)** |
...
1. On the **Container** tab, fill out the following information, and then select **Ingress (8)** tab:
